...
app.get('/api/inventory/:id/export', (req, res) => {
  const id = req.params.id;
  db.get(`SELECT * FROM api_inventory WHERE id = ?`, [id], (err, api) => {
    if (err) return res.status(500).json({ error: err.message });
    if (!api) return res.status(404).json({ error: 'API not found' });

    db.all(`SELECT * FROM api_endpoints WHERE api_inventory_id = ? ORDER BY method, path`, [id], (err, endpoints) => {
      if (err) return res.status(500).json({ error: err.message });

      db.all(`SELECT d.*, t.name as target_name FROM api_dependencies d JOIN api_inventory t ON d.target_api_id = t.id WHERE d.source_api_id = ?`, [id], (err, deps) => {
        if (err) return res.status(500).json({ error: err.message });

        // Generate Markdown
        let md = `# ${api.name}\n\n`;
        md += `> ${api.description || 'No description'}\n\n`;
        md += `| Property | Value |\n|----------|-------|\n`;
        md += `| Base URL | \`${api.base_url || 'N/A'}\` |\n`;
        md += `| Auth | ${api.auth_type || 'none'} |\n`;
        md += `| Status | ${api.status} |\n`;
        md += `| Environment | ${api.environment_type || 'unknown'} |\n`;
        md += `| Schema | ${api.detected_schema || 'unknown'} |\n`;
        md += `| Project | ${api.project} |\n`;
        md += `| Total Calls | ${api.total_calls || 0} |\n`;
        md += `| Avg Response | ${api.avg_response_time > 0 ? Math.round(api.avg_response_time) + 'ms' : 'N/A'} |\n\n`;
        ...
        if (endpoints && endpoints.length > 0) {
          ...
          endpoints.forEach(ep => {
            ...
            if (ep.error_codes) md += `**Error Codes:** ${ep.error_codes}\n\n`;
            if (ep.notes) md += `**Notes:** ${ep.notes}\n\n`;
            md += `---\n\n`;
          });
        }
        ...
        if (deps && deps.length > 0) {
          md += `## Dependencies\n\n`;
          deps.forEach(d => {
            md += `- **${d.dependency_type}** → ${d.target_name}`;
        ...
        md += `---\n*Generated by API Client on ${new Date().toISOString()}*\n`;

        res.json({ markdown: md, filename: `${api.name.replace(/[^a-zA-Z0-9]/g, '_')}_docs.md` });
      });
    });
  });
});
